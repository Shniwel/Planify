...
const CATEGORIES = {
  "Production": { color: "#10b981", icon: "🏭", sub: ["Ligne 1", "Ligne 2"] },
  "Maintenance": { color: "#f59e0b", icon: "🔧", sub: ["Maintenance"] }
};
...
// ── Email — utilise les schedules FRAIS passés en paramètre ──────────────────
async function sendEmail(employee, freshSchedules, action, toast) {
  try {
    toast(`📧 Envoi de l'email à ${employee.name}…`, "info");
    const res = await fetch("/api/send-email", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ employee, schedules: freshSchedules, action }),
    });
...
function Toast({toasts,remove}){
  ...
  return(
    <div style={{position:"fixed",bottom:24,right:24,zIndex:9999,display:"flex",flexDirection:"column",gap:10}}>
      {toasts.map(t=>(
        <div key={t.id} onClick={()=>remove(t.id)} style={{background:bg[t.type]||"#6366f1",color:"#fff",padding:"12px 18px",borderRadius:12,boxShadow:"0 8px 32px rgba(0,0,0,.2)",cursor:"pointer",display:"flex",alignItems:"center",gap:10,maxWidth:340,animation:"slideIn .3s cubic-bezier(.34,1.56,.64,1)",fontFamily:"'DM Sans',sans-serif",fontSize:14}}>
...
function Modal({title,children,onClose}){
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(15,15,25,.65)",backdropFilter:"blur(6px)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000,padding:16}} onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div style={{background:"#fff",borderRadius:20,padding:"32px 36px",width:"100%",maxWidth:520,boxShadow:"0 32px 64px rgba(0,0,0,.18)",animation:"popIn .25s cubic-bezier(.34,1.56,.64,1)"}}>
...
const IS={width:"100%",boxSizing:"border-box",padding:"10px 14px",border:"2px solid #e5e7eb",borderRadius:10,fontSize:14,color:"#1f2937",outline:"none",transition:"border .2s",fontFamily:"'DM Sans',sans-serif",background:"#fafafa"};
function Input(props){return <input {...props} style={{...IS,...(props.style||{})}} onFocus={e=>e.target.style.borderColor="#6366f1"} onBlur={e=>e.target.style.borderColor="#e5e7eb"}/>;}
function Sel({children,...props}){return <select {...props} style={{...IS,appearance:"none",cursor:"pointer",...(props.style||{})}} onFocus={e=>e.target.style.borderColor="#6366f1"} onBlur={e=>e.target.style.borderColor="#e5e7eb"}>{children}</select>;}

function Btn({children,variant="primary",small,...props}){
  const base={border:"none",borderRadius:small?8:12,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",fontWeight:600,transition:"all .15s",padding:small?"6px 14px":"12px 22px",fontSize:small?12:14,display:"inline-flex",alignItems:"center",gap:6};
  const styles={primary:{background:"linear-gradient(135deg,#6366f1,#8b5cf6)",color:"#fff",boxShadow:"0 4px 14px rgba(99,102,241,.35)"},secondary:{background:"#f1f5f9",color:"#374151"},danger:{background:"#fee2e2",color:"#dc2626"},ghost:{background:"transparent",color:"#6366f1"}};
  return <button {...props} style={{...base,...styles[variant],...(props.disabled?{opacity:.5,cursor:"not-allowed"}:{})}} onMouseEnter={e=>!props.disabled&&(e.currentTarget.style.transform="translateY(-1px)")} onMouseLeave={e=>e.currentTarget.style.transform="none"}>{children}</button>;
}
...
function CatBadge({category,subcategory,small}){
  ...
  return(
    <div style={{display:"inline-flex",alignItems:"center",gap:4}}>
      <span style={{fontSize:small?10:11,padding:small?"2px 6px":"3px 10px",borderRadius:20,background:cat.color+"18",color:cat.color,fontWeight:700,border:`1px solid ${cat.color}30`}}>{cat.icon} {category}</span>
      {subcategory&&subcategory!==category&&<span style={{fontSize:small?10:11,padding:small?"2px 6px":"3px 10px",borderRadius:20,background:"#f1f5f9",color:"#64748b",fontWeight:600}}>{subcategory}</span>}
    </div>
  );
}
...
function Login({onLogin}){
  ...
  const submit=()=>{setLoading(true);setTimeout(()=>{if(email==="admin@planify.fr"&&pass==="admin123")onLogin();else setErr("Identifiants incorrects.");setLoading(false);},800);};
  return(
    <div style={{minHeight:"100vh",background:"linear-gradient(135deg,#0f0c29,#302b63,#24243e)",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'DM Sans',sans-serif"}}>
      ...
      <div style={{background:"rgba(255,255,255,.05)",backdropFilter:"blur(20px)",border:"1px solid rgba(255,255,255,.12)",borderRadius:24,padding:"48px 40px",width:"100%",maxWidth:420,boxShadow:"0 32px 64px rgba(0,0,0,.5)"}}>
        ...
        <Field label={<span style={{color:"rgba(255,255,255,.7)"}}>Email</span>}><Input value={email} onChange={e=>setEmail(e.target.value)} style={{background:"rgba(255,255,255,.08)",border:"1px solid rgba(255,255,255,.15)",color:"#fff"}} type="email"/></Field>
        <Field label={<span style={{color:"rgba(255,255,255,.7)"}}>Mot de passe</span>}><Input value={pass} onChange={e=>setPass(e.target.value)} style={{background:"rgba(255,255,255,.08)",border:"1px solid rgba(255,255,255,.15)",color:"#fff"}} type="password" onKeyDown={e=>e.key==="Enter"&&submit()}/></Field>
        {err&&<p style={{color:"#fca5a5",fontSize:13,margin:"-8px 0 16px",textAlign:"center"}}>{err}</p>}
        <button onClick={submit} disabled={loading} style={{width:"100%",padding:"14px",border:"none",borderRadius:12,cursor:"pointer",background:loading?"rgba(99,102,241,.4)":"linear-gradient(135deg,#6366f1,#8b5cf6)",color:"#fff",fontSize:15,fontWeight:700,fontFamily:"'DM Sans',sans-serif",boxShadow:"0 8px 24px rgba(99,102,241,.4)"}}>{loading?"Connexion…":"Se connecter"}</button>
...
function EmployeeModal({employee,onSave,onClose}){
  ...
  return(
    <Modal title={employee?"Modifier l'employé":"Nouvel employé"} onClose={onClose}>
      ...
      <Field label="Poste"><Sel value={form.role||""} onChange={set("role")}><option value="">— Sélectionner —</option>{["Manager","Opérateur","Technicien","Chef d'équipe","Agent de maintenance","Superviseur"].map(r=><option key={r}>{r}</option>)}</Sel></Field>
      <div style={{borderTop:"1px solid #f1f5f9",margin:"8px 0 16px",paddingTop:16}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          <Field label="Catégorie">
            <Sel value={form.category||""} onChange={e=>{const v=e.target.value;setForm(p=>({...p,category:v,subcategory:v?CATEGORIES[v]?.sub[0]||"":""}));}}>
      ...
      {form.category&&(
        <div style={{background:CATEGORIES[form.category]?.color+"10",border:`1px solid ${CATEGORIES[form.category]?.color}30`,borderRadius:10,padding:"10px 14px",marginBottom:16,display:"flex",alignItems:"center",gap:8}}>
          ...
          <div><p style={{margin:0,fontSize:13,fontWeight:700,color:CATEGORIES[form.category]?.color}}>{form.category}</p>{form.subcategory&&<p style={{margin:0,fontSize:11,color:"#64748b"}}>{form.subcategory}</p>}</div>
        </div>
      )}
      <div style={{display:"flex",gap:10,justifyContent:"flex-end",marginTop:8}}>
        <Btn variant="secondary" onClick={onClose}>Annuler</Btn>
        <Btn disabled={!valid||saving} onClick={async()=>{setSaving(true);await onSave(form);setSaving(false);}}>{saving?"Sauvegarde…":employee?"Enregistrer":"Ajouter"}</Btn>
      </div>
    </Modal>
  );
}
    ...
    <Modal title={schedule?"Modifier le créneau":"Nouveau créneau"} onClose={onClose}>
      <Field label="Employé">
        <Sel value={form.employee_id} onChange={e=>setForm(p=>({...p,employee_id:e.target.value}))}>
          {employees.map(emp=><option key={emp.id} value={emp.id}>{emp.name}{emp.category?` — ${emp.subcategory||emp.category}`:""}</option>)}
        </Sel>
      </Field>
      ...
      <div style={{display:"flex",gap:10,justifyContent:"flex-end",marginTop:8}}>
        <Btn variant="secondary" onClick={onClose}>Annuler</Btn>
        <Btn disabled={!valid||saving} onClick={async()=>{setSaving(true);await onSave(form);setSaving(false);}}>{saving?"Sauvegarde…":schedule?"Enregistrer":"Ajouter"}</Btn>
      </div>
    </Modal>
  );
}
...
function EmailPreviewModal({employee,schedules,onClose,onSendNow}){
  const empSch=schedules.filter(s=>s.employee_id===employee.id);
  ...
  return(
    <Modal title="Aperçu & envoi de l'email" onClose={onClose}>
      <div style={{background:"#f8fafc",borderRadius:12,padding:20,fontFamily:"monospace",fontSize:13,lineHeight:1.7,maxHeight:320,overflowY:"auto"}}>
        ...
        <p style={{margin:0,color:"#64748b"}}>Objet : <strong>📅 Votre planning</strong></p>
        <hr style={{border:"none",borderTop:"1px solid #e5e7eb",margin:"12px 0"}}/>
        <p>Bonjour <strong>{employee.name.split(" ")[0]}</strong>,</p>
        {empSch.length===0?<p style={{color:"#94a3b8"}}>Aucun créneau assigné.</p>:empSch.map(s=>(
        ...
        <p>⏱ Total : <strong>{totalHours(empSch)}h</strong></p>
      </div>
      <div style={{display:"flex",gap:10,justifyContent:"flex-end",marginTop:16}}>
        <Btn variant="secondary" onClick={onClose}>Fermer</Btn>
        <Btn disabled={sending} onClick={async()=>{setSending(true);await onSendNow(employee,empSch);setSending(false);onClose();}}>{sending?"Envoi…":"📧 Envoyer maintenant"}</Btn>
      </div>
    </Modal>
  );
}
...
function StatCard({icon,label,value,color}){
  return(
    <div style={{background:"#fff",borderRadius:16,padding:"20px 24px",boxShadow:"0 2px 12px rgba(0,0,0,.06)",display:"flex",alignItems:"center",gap:16,flex:"1 1 160px"}}>
      ...
      <div><p style={{margin:0,fontSize:13,color:"#94a3b8",fontWeight:500}}>{label}</p><p style={{margin:0,fontSize:26,fontWeight:800,color:"#1e1b4b"}}>{value}</p></div>
    </div>
  );
}
  ...
  return(
    <div style={{overflowX:"auto"}}>
      <div style={{minWidth:700}}>
        ...
        {employees.map(emp=>{
          const empSch=schedules.filter(s=>String(s.employee_id)===String(emp.id));
          const c=emp.category?CATEGORIES[emp.category]?.color:emp.color||"#6366f1";
          return(
            <div key={emp.id} style={{display:"grid",gridTemplateColumns:"90px repeat(7,1fr)",gap:4,marginBottom:4,alignItems:"stretch"}}>
              <div style={{display:"flex",alignItems:"center",gap:6,padding:"0 4px"}}>
                ...
                <div>
                  <span style={{fontSize:10,fontWeight:700,color:"#475569",display:"block",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{emp.name.split(" ")[0]}</span>
        ...
        {employees.length===0&&<p style={{textAlign:"center",color:"#94a3b8",padding:40}}>Aucun employé à afficher</p>}
      </div>
    </div>
  );
}
...
function EmployeeCard({emp,schedules,onEdit,onDelete,onEmailPreview}){
  ...
  const c=emp.category?CATEGORIES[emp.category]?.color:emp.color||"#6366f1";
  return(
    <div style={{background:"#fff",borderRadius:16,padding:20,boxShadow:"0 2px 12px rgba(0,0,0,.06)",border:`2px solid ${c}25`,transition:"transform .2s,box-shadow .2s"}} onMouseEnter={e=>{e.currentTarget.style.transform="translateY(-2px)";e.currentTarget.style.boxShadow="0 8px 24px rgba(0,0,0,.1)";}} onMouseLeave={e=>{e.currentTarget.style.transform="none";e.currentTarget.style.boxShadow="0 2px 12px rgba(0,0,0,.06)";}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
        <div style={{display:"flex",gap:12,alignItems:"center"}}>
          <div style={{width:46,height:46,borderRadius:14,background:c,display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontWeight:800,fontSize:18}}>{emp.name[0]}</div>
          <div><p style={{margin:0,fontWeight:700,color:"#1e1b4b",fontSize:15}}>{emp.name}</p><p style={{margin:0,fontSize:12,color:"#94a3b8"}}>{emp.role||"—"}</p></div>
        </div>
      ...
      {emp.category&&<div style={{marginTop:10}}><CatBadge category={emp.category} subcategory={emp.subcategory}/></div>}
      ...
      <div style={{marginTop:10,display:"flex",gap:6,flexWrap:"wrap"}}>
        {empSch.length===0?<span style={{fontSize:11,color:"#cbd5e1",fontStyle:"italic"}}>Aucun créneau</span>:empSch.map(s=><span key={s.id} style={{fontSize:11,padding:"3px 8px",borderRadius:20,background:c+"18",color:c,fontWeight:600}}>{s.day.slice(0,3)} {s.start}–{s.end}</span>)}
      </div>
      <div style={{marginTop:12,paddingTop:10,borderTop:"1px solid #f1f5f9",display:"flex",justifyContent:"space-between"}}>
        ...
        <span style={{fontSize:12,fontWeight:700,color:c}}>{totalHours(empSch)}h / semaine</span>
      </div>
    </div>
  );
}

const NAV=[{id:"dashboard",icon:"◉",label:"Tableau de bord"},{id:"employees",icon:"👥",label:"Employés"},{id:"calendar",icon:"📅",label:"Calendrier"},{id:"schedules",icon:"🕐",label:"Horaires"}];
    ...
    <div style={{width:collapsed?68:220,background:"linear-gradient(180deg,#1e1b4b,#312e81)",height:"100vh",display:"flex",flexDirection:"column",transition:"width .3s cubic-bezier(.4,0,.2,1)",boxShadow:"4px 0 20px rgba(0,0,0,.15)",flexShrink:0,position:"sticky",top:0}}>
      ...
      <nav style={{flex:1,padding:"12px 10px"}}>
        {NAV.map(item=>{
          ...
          return(
            <button key={item.id} onClick={()=>onNav(item.id)} style={{width:"100%",display:"flex",alignItems:"center",gap:12,padding:"11px 12px",borderRadius:12,border:"none",cursor:"pointer",marginBottom:4,transition:"all .15s",background:isActive?"rgba(99,102,241,.3)":"transparent",color:isActive?"#a5b4fc":"rgba(255,255,255,.55)",fontFamily:"'DM Sans',sans-serif",fontSize:14,fontWeight:isActive?600:400,textAlign:"left"}} onMouseEnter={e=>!isActive&&(e.currentTarget.style.background="rgba(255,255,255,.06)")} onMouseLeave={e=>!isActive&&(e.currentTarget.style.background="transparent")}>
...
export default function App(){
  ...
  // ── Charger TOUT depuis Supabase, retourne les données fraîches ─────────────
  const loadData=useCallback(async()=>{
    setLoading(true);
    const[{data:emps,error:e1},{data:schs,error:e2}]=await Promise.all([
      supabase.from("employees").select("*").order("name"),
      supabase.from("schedules").select("*"),
    ]);
    const freshEmps=emps||[];
    const freshSchs=schs||[];
    setEmployees(freshEmps);
    setSchedules(freshSchs);
    setLoading(false);
    return{emps:freshEmps,schs:freshSchs};
  },[]);
  ...
  const saveEmployee=async(form)=>{
    ...
    if(form.id){
      await supabase.from("employees").update({name:form.name,email:form.email,phone:form.phone,role:form.role,category:form.category||null,subcategory:form.subcategory||null,color:catColor}).eq("id",form.id);
      toast(`${form.name} mis à jour ✅`,"success");
    }else{
      await supabase.from("employees").insert({name:form.name,email:form.email,phone:form.phone,role:form.role,category:form.category||null,subcategory:form.subcategory||null,color:catColor});
  ...
  const saveSchedule=async(form)=>{
    ...
    if(isEdit){
      await supabase.from("schedules").update({
        employee_id:form.employee_id,day:form.day,start:form.start,end:form.end
      }).eq("id",form.id);
    }else{
    ...
    // ✅ Recharger les données FRAÎCHES avant d'envoyer l'email
    const{schs:freshSchs}=await loadData();
    setSchModal(null);
    toast(`Créneau ${isEdit?"modifié":"ajouté"} pour ${emp?.name||"?"} ✅`,"success");

    // ✅ Utiliser les schedules frais pour l'email
    if(emp){
      const empFreshSchs=freshSchs.filter(s=>String(s.employee_id)===String(emp.id));
      setTimeout(()=>sendEmail(emp,empFreshSchs,isEdit?"modification":"creation",toast),300);
    }
  };

  const deleteSchedule=async(id)=>{
    const sch=schedules.find(s=>s.id===id);
    const emp=employees.find(e=>String(e.id)===String(sch?.employee_id));
  ...
  return(
    <div style={{display:"flex",minHeight:"100vh",background:"#f8faff",fontFamily:"'DM Sans',sans-serif"}}>
          ...
          {page==="dashboard"&&(
            <div>
              <div style={{display:"flex",gap:16,flexWrap:"wrap",marginBottom:28}}>
                ...
                <StatCard icon="🏭" label="Production" value={employees.filter(e=>e.category==="Production").length} color="#10b981"/>
                <StatCard icon="🔧" label="Maintenance" value={employees.filter(e=>e.category==="Maintenance").length} color="#f59e0b"/>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(280px,1fr))",gap:16,marginBottom:24}}>
                {Object.entries(CATEGORIES).map(([catName,cat])=>{
                  ...
                  const catSchs=schedules.filter(s=>catEmps.some(e=>String(e.id)===String(s.employee_id)));
                  return(
                    <div key={catName} style={{background:"#fff",borderRadius:16,padding:20,boxShadow:"0 2px 12px rgba(0,0,0,.06)",borderLeft:`4px solid ${cat.color}`}}>
                      <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14}}>
                        ...
                        <div><h3 style={{margin:0,fontSize:16,fontWeight:700,color:"#1e1b4b"}}>{catName}</h3><p style={{margin:0,fontSize:12,color:"#94a3b8"}}>{catEmps.length} employé{catEmps.length!==1?"s":""}</p></div>
                      </div>
                      {cat.sub.map(sub=>{
                        const subEmps=catEmps.filter(e=>e.subcategory===sub);
                        return(<div key={sub} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"6px 10px",borderRadius:8,background:cat.color+"08",marginBottom:4}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(280px,1fr))",gap:20}}>
                <div style={{background:"#fff",borderRadius:16,padding:24,boxShadow:"0 2px 12px rgba(0,0,0,.06)"}}>
                  ...
                  {employees.map(emp=>{
                    const h=totalHours(schedules.filter(s=>String(s.employee_id)===String(emp.id)));
                    const c=emp.category?CATEGORIES[emp.category]?.color:emp.color||"#6366f1";
                    return(<div key={emp.id} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 0",borderBottom:"1px solid #f8fafc"}}>
                      ...
                      <div style={{flex:1}}><p style={{margin:0,fontSize:13,fontWeight:600,color:"#374151"}}>{emp.name}</p><div style={{marginTop:2}}>{emp.category&&<CatBadge category={emp.category} subcategory={emp.subcategory} small/>}</div></div>
                  ...
                  {DAYS.map(day=>{
                    ...
                    return(<div key={day} style={{marginBottom:10}}>
                      ...
                      <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
                        {slots.map(s=>{const emp=employees.find(e=>String(e.id)===String(s.employee_id));const c=emp?.category?CATEGORIES[emp.category]?.color:emp?.color||"#6366f1";return emp?(<span key={s.id} style={{fontSize:11,padding:"2px 8px",borderRadius:20,background:c+"18",color:c,fontWeight:600}}>{emp.name.split(" ")[0]} · {s.start}–{s.end}</span>):null;})}
                      </div>
                    </div>);
                  })}
                </div>
            ...
            <div>
              <div style={{display:"flex",gap:8,marginBottom:20,flexWrap:"wrap"}}>
                {["all",...Object.keys(CATEGORIES)].map(cat=>(
                  <button key={cat} onClick={()=>setFilterCat(cat)} style={{padding:"8px 16px",borderRadius:20,border:"none",cursor:"pointer",fontSize:13,fontWeight:600,fontFamily:"'DM Sans',sans-serif",background:filterCat===cat?"linear-gradient(135deg,#6366f1,#8b5cf6)":"#fff",color:filterCat===cat?"#fff":"#64748b",boxShadow:filterCat===cat?"0 4px 14px rgba(99,102,241,.35)":"0 1px 4px rgba(0,0,0,.06)"}}>
                    {cat==="all"?"👥 Tous":`${CATEGORIES[cat]?.icon} ${cat}`} <span style={{fontSize:11,opacity:.8}}>({cat==="all"?employees.length:employees.filter(e=>e.category===cat).length})</span>
                  </button>
                ))}
              </div>
          ...
          {page==="schedules"&&(
            <div>
              ...
              {Object.entries(CATEGORIES).map(([catName,cat])=>{
                ...
                return(
                  <div key={catName}>
                    {filterCat==="all"&&<div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12,marginTop:8}}><span style={{fontSize:20}}>{cat.icon}</span><h3 style={{margin:0,fontSize:16,fontWeight:700,color:cat.color}}>{catName}</h3><div style={{flex:1,height:1,background:cat.color+"20"}}/></div>}
                    {catEmps.map(emp=>{
                      const empSch=schedules.filter(s=>String(s.employee_id)===String(emp.id));
                      return(<div key={emp.id} style={{background:"#fff",borderRadius:16,padding:20,boxShadow:"0 2px 12px rgba(0,0,0,.06)",marginBottom:12,borderLeft:`3px solid ${cat.color}`}}>
                        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
                          <div style={{display:"flex",alignItems:"center",gap:10}}>
                            <div style={{width:38,height:38,borderRadius:10,background:cat.color,display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontWeight:700}}>{emp.name[0]}</div>
                            <div><p style={{margin:0,fontWeight:700,color:"#1e1b4b"}}>{emp.name}</p><div style={{marginTop:2}}><CatBadge category={emp.category} subcategory={emp.subcategory} small/></div></div>
                          </div>
                        ...
                        {empSch.length===0?<p style={{color:"#cbd5e1",fontSize:13,fontStyle:"italic",margin:0}}>Aucun créneau assigné</p>:(
                          <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
                            {empSch.sort((a,b)=>DAYS.indexOf(a.day)-DAYS.indexOf(b.day)).map(s=>(
                              <div key={s.id} style={{padding:"8px 14px",borderRadius:12,background:cat.color+"12",border:`1.5px solid ${cat.color}30`,display:"flex",alignItems:"center",gap:10}}>
                                <span style={{fontSize:12,fontWeight:700,color:cat.color}}>{s.day}</span>
              ...
              {employees.filter(e=>!e.category).filter(e=>filterCat==="all").map(emp=>{
                const empSch=schedules.filter(s=>String(s.employee_id)===String(emp.id));
      ...
      {schModal&&<ScheduleModal
        schedule={schModal.mode==="edit"?schModal.data:schModal.preselect?{employee_id:schModal.preselect,day:"Lundi",start:"08:00",end:"16:00"}:null}
        employees={employees} onSave={saveSchedule} onClose={()=>setSchModal(null)}/>}
      {emailModal&&<EmailPreviewModal employee={emailModal} schedules={schedules} onClose={()=>setEmailModal(null)} onSendNow={async(emp,schs)=>sendEmail(emp,schs,"envoi_manuel",toast)}/>}
