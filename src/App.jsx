...
function StatCard({icon,label,value,color}){
  return(
    <div style={{background:"#fff",borderRadius:16,padding:"20px 24px",boxShadow:"0 2px 12px rgba(0,0,0,.06)",display:"flex",alignItems:"center",gap:16,flex:"1 1 180px"}}>
          ...
          return(
            <div key={emp.id} style={{display:"grid",gridTemplateColumns:"80px repeat(7,1fr)",gap:4,marginBottom:4,alignItems:"stretch"}}>
              <div style={{display:"flex",alignItems:"center",gap:8,padding:"0 4px"}}>
                ...
                <span style={{fontSize:11,fontWeight:600,color:"#475569",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{emp.name.split(" ")[0]}</span>
              </div>
              {DAYS.map(day=>{
                const slot=empSch.find(s=>s.day===day);
                return(
                  <div key={day} style={{minHeight:60,borderRadius:10,position:"relative",background:slot?emp.color+"18":"#f8fafc",border:slot?`1.5px solid ${emp.color}40`:"1.5px solid #f1f5f9"}}>
                    {slot&&(
...
function EmployeeCard({emp,schedules,onEdit,onDelete,onEmailPreview}){
  const empSch=schedules.filter(s=>s.employee_id===emp.id);
  return(
    <div style={{background:"#fff",borderRadius:16,padding:20,boxShadow:"0 2px 12px rgba(0,0,0,.06)",border:`2px solid ${emp.color}25`,transition:"transform .2s,box-shadow .2s"}}
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
        <div style={{display:"flex",gap:12,alignItems:"center"}}>
          <div style={{width:46,height:46,borderRadius:14,background:emp.color,display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontWeight:800,fontSize:18}}>{emp.name[0]}</div>
          <div>
      ...
      <div style={{marginTop:12,display:"flex",gap:6,flexWrap:"wrap"}}>
        {empSch.length===0?<span style={{fontSize:11,color:"#cbd5e1",fontStyle:"italic"}}>Aucun créneau</span>
          :empSch.map(s=><span key={s.id} style={{fontSize:11,padding:"3px 8px",borderRadius:20,background:emp.color+"18",color:emp.color,fontWeight:600}}>{s.day.slice(0,3)} {s.start}–{s.end}</span>)}
      </div>
...
export default function App(){
  ...
  const saveEmployee=async(form)=>{
    ...
    if(form.id){
      await supabase.from("employees").update({name:form.name,email:form.email,phone:form.phone,role:form.role}).eq("id",form.id);
      toast(`${form.name} mis à jour ✅`,"success");
    }else{
      await supabase.from("employees").insert({name:form.name,email:form.email,phone:form.phone,role:form.role,color:COLORS[colorIndex]});
      ...
      <main style={{flex:1,overflow:"auto"}}>
        ...
        {loading?(
          <div style={{display:"flex",alignItems:"center",justifyContent:"center",height:"60vh",flexDirection:"column",gap:16}}>
            ...
            <p style={{color:"#94a3b8",fontSize:14}}>Chargement des données…</p>
          </div>
        ):(
        <div style={{padding:"28px 32px",animation:"fadeUp .35s ease"}}>
                ...
                <div style={{background:"#fff",borderRadius:16,padding:24,boxShadow:"0 2px 12px rgba(0,0,0,.06)"}}>
                  ...
                  {employees.map(emp=>{
                    ...
                    return(<div key={emp.id} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 0",borderBottom:"1px solid #f8fafc"}}>
                      ...
                      <div style={{flex:1}}><p style={{margin:0,fontSize:13,fontWeight:600,color:"#374151"}}>{emp.name}</p><p style={{margin:0,fontSize:11,color:"#94a3b8"}}>{emp.role||"—"}</p></div>
                  ...
                  {DAYS.map(day=>{
                    ...
                    return(<div key={day} style={{marginBottom:10}}>
                      ...
                      <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
                        {slots.map(s=>{const emp=employees.find(e=>e.id===s.employee_id);return emp?(<span key={s.id} style={{fontSize:11,padding:"2px 8px",borderRadius:20,background:emp.color+"18",color:emp.color,fontWeight:600}}>{emp.name.split(" ")[0]} · {s.start}–{s.end}</span>):null;})}
                      </div>
                    </div>);
                  })}
                </div>
          ...
          {page==="employees"&&(
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(320px,1fr))",gap:16}}>
              {employees.map(emp=>(<EmployeeCard key={emp.id} emp={emp} schedules={schedules} onEdit={e=>setEmpModal({mode:"edit",data:e})} onDelete={deleteEmployee} onEmailPreview={e=>setEmailModal(e)}/>))}
              {employees.length===0&&(<div style={{gridColumn:"1/-1",textAlign:"center",padding:60,color:"#94a3b8"}}><div style={{fontSize:48,marginBottom:12}}>👥</div><p>Aucun employé. Cliquez sur <strong>+ Ajouter un employé</strong>.</p></div>)}
            </div>
          )}
          {page==="calendar"&&(
            <div style={{background:"#fff",borderRadius:20,padding:24,boxShadow:"0 2px 12px rgba(0,0,0,.06)"}}>
              <CalendarView employees={employees} schedules={schedules} onEdit={s=>setSchModal({mode:"edit",data:s})} onDelete={deleteSchedule}/>
            </div>
          )}
          {page==="schedules"&&(
            <div>
              {employees.map(emp=>{
                const empSch=schedules.filter(s=>s.employee_id===emp.id);
                return(<div key={emp.id} style={{background:"#fff",borderRadius:16,padding:20,boxShadow:"0 2px 12px rgba(0,0,0,.06)",marginBottom:16}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
                    <div style={{display:"flex",alignItems:"center",gap:10}}>
                      ...
                      <div><p style={{margin:0,fontWeight:700,color:"#1e1b4b"}}>{emp.name}</p><p style={{margin:0,fontSize:12,color:"#94a3b8"}}>{totalHours(empSch)}h cette semaine</p></div>
                    </div>
                  ...
                  {empSch.length===0?<p style={{color:"#cbd5e1",fontSize:13,fontStyle:"italic",margin:0}}>Aucun créneau assigné</p>:(
                    <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
                      {empSch.sort((a,b)=>DAYS.indexOf(a.day)-DAYS.indexOf(b.day)).map(s=>(
                        <div key={s.id} style={{padding:"8px 14px",borderRadius:12,background:emp.color+"12",border:`1.5px solid ${emp.color}30`,display:"flex",alignItems:"center",gap:10}}>
